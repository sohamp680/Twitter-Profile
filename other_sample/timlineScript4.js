...
//console.log('user'+1)
var newUser=[];
newUser=users.forEach((user,i,newUser) => {
    newUser[i]= (({ userName,displayName,joinedDate,followingCount,followerCount,avatarURL,coverPhotoURL }) => ({ userName,displayName,joinedDate,followingCount,followerCount,avatarURL,coverPhotoURL }))(user);
    return newUser[i];
 //   console.log(tweetsOfUser[i]);
}
);  
console.log(newUser);

// users.map((user) => {
//     let newUser = (({ userName,displayName,joinedDate,followingCount,followerCount,avatarURL,coverPhotoURL }) =>
//      ({ userName,displayName,joinedDate,followingCount,followerCount,avatarURL,coverPhotoURL}))(user);
...
var tweetsOfUser=[];
tweetsOfUser[i]=user.tweets;
Object.assign(tweetsOfUser[i], newUser);
...
//var newUser1 =_.pick(user1,'userName','displayName','joinedDate','followingCount','followerCount','avatarURL','coverPhotoURL');

//var tweetsOfUser1=users[0].tweets;
//var tweetsOfUser2=users[1].tweets;
//console.log(numberOfUsers);
var numberOfTweetOfUser=user.tweets.length;
//console.log(numberOfTweetOfUser);
 for (i=0;i<numberOfTweetOfUser;i++){
     var tweetsOfUser11=Object.assign(tweetsOfUser1[i], newUser1);
     var tweetsOfUser22=Object.assign(tweetsOfUser2[i], newUser2);
 }
 
//let anothObject = {newUser1,tweetsOfUser1 };
